...
foreach(i=1:num_studies) %do% {
  k <- quantile(data$lnt[data$status==1 & data$trialid==i], c(0, 0.25, 0.5, 0.75,1))
...
data$t1 <- data$lnt-data$k2#knot1
data$t2 <- data$lnt-data$k1  #min
data$t3 <- data$lnt-data$k5  #max
data$t4 <- data$lnt-data$k3#knot2
data$t5 <- data$lnt-data$k4#knot 3
#t2(kmin) t3(kmax) is used in every  v
foreach(i=1:pts) %do% {
  data$a[i] <- max(c(data$t1[i], 0))
  data$b[i] <- max(c(data$t2[i], 0))#min
  data$c[i] <- max(c(data$t3[i], 0))#max
  data$e[i] <- max(c(data$t4[i], 0))
  data$ff[i] <- max(c(data$t5[i], 0))
  #3 knots, 3 phi
  data$phi1[i] <- (data$k5[i]-data$k2[i])/(data$k5[i]-data$k1[i]) #first knot kmax-k1st knot/kmax-kmin
  data$phi2[i] <- (data$k5[i]-data$k3[i])/(data$k5[i]-data$k1[i]) #second knot:kmax-k2nd knot/kmax-kmin
  data$phi3[i] <- (data$k5[i]-data$k4[i])/(data$k5[i]-data$k1[i]) #third knot:kmax-k3rd knot/kmax-kmin
  #3 knots, 4 v
  ...
  data$v3[i] <- data$ff[i]^3 - (data$phi3[i]*(data$b[i]^3)) - ((1-data$phi3[i])*(data$c[i]^3))  #3rd knot
}
...
#3. v2->u2
df4 <- data.frame(trialid=NA, v2u0=NA, v2u1n=NA, u1nu1n=NA) #create new var for u2 calc.
foreach(i=1:num_studies) %do% {
...
foreach(i=1:pts) %do% {
  ...
  data$hh[i] <- data$v3[i]*data$u0[i]#v3u0
  data$aa[i] <- data$v3[i]*data$u1n[i]#v3u1n
  data$jj[i] <- data$v3[i]*data$u2n[i]#v3u2n
  data$kk[i] <- data$u2n[i]*data$u2n[i]#u2nu2n
  
}

#4. normalising u2, prepare to use it to calculate v3->u3
df6 <- data.frame(trialid=NA, v3u0=NA, v3u1n=NA, v3u2n=NA,u2nu2n=NA)
foreach(i=1:num_studies) %do% {
 
  v3u0 <- sum(data$hh[data$trialid==i])
  v3u1n <- sum(data$aa[data$trialid==i])
  v3u2n <- sum(data$jj[data$trialid==i])
  u2nu2n <- sum(data$kk[data$trialid==i])
...
foreach(i=1:pts) %do% {
  data$u3n[i] <- (data$u3[i]-data$mu3[i])/data$sdu3[i]#normalising u3
  ...
  data$o[i] <- (3*(data$ff[i]^2)) - ((3*data$phi3[i])*(data$b[i]^2))- (3*(1-data$phi3[i])*(data$c[i]^2))
  data$p[i] <- data$sdu3[i]
  data$du3n[i] <- ((1/data$p[i])*data$o[i]) - (((data$v3u0[i]/data$u0u0[i])/data$p[i])*data$du0[i]) - (((data$v3u1n[i]/data$u1nu1n[[i]])/data$p[i])*data$du1n[i]) - (((data$v3u2n[i]/data$u2nu2n[[i]])/data$p[i])*data$du2n[i])
}
...
bugs_data <- list(d=data$status, trt2=data$trt2, trt3=data$trt3, trt4=data$trt4, 
                  ...
                  u0=data$u0, u1n=data$u1n, u2n=data$u2n,u3n=data$u3n,  du0=data$du0,
                  du1n=data$du1n, du2n=data$du2n,du3n=data$du3n, Ntrials=num_studies, offset=offset,
                  k0=knots$k1[1], k1=knots$k2[1], k2=knots$k3[1], k3=knots$k4[1],k4=knots$k5[1],#reference study:study 1
                  ...
                  v3w0=df6$v3u0[1], v3w1n=df6$v3u1n[1],v3w2n=df6$v3u2n[1],
                  w2nw2n=df6$u2nu2n[1], meanw3=df7$mean[1], sdw3=df7$sd[1],
                  nt=nt,maxt=maxt)
...
start_time <- system.time(
  bugs.object<-bugs(data=bugs_data, inits=inits, 
                    ...
                    model.file="RP model - 3 knots.txt", clearWD=F, 
...
write.csv(results, "RP_3k_results_wk_40.csv")#fully extrapolated for 40 yrs
write.csv(results, "RP_3k_results_mo_40.csv")#fully extrapolated for 40 yrs
